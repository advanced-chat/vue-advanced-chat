...
function handleTextFormatting(content, { linkify, linkOptions, singleLine, users, ...formatting }) {
  ...
  message.forEach(m => {
    const { html, value } = replaceEmojiByElement(content, m.value, { singleLine })
...
function containsOnlyEmojis(content) {
  const onlyEmojis = content.replace(
    new RegExp('[\u0000-\u1eeff]', 'g'),
    ''
  )
  const visibleChars = content.replace(
    new RegExp('[\n\rs]+|( )+', 'g'),
    ''
  )
