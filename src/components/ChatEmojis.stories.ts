...
export const ClickEmits: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    const first = canvasElement.querySelector('.acc-autocomplete-item') as HTMLElement
...
export const HoverHighlightsItem: Story = {
  ...
  play: async ({ canvasElement }) => {
    const items = canvasElement.querySelectorAll('.acc-autocomplete-item')
    expect(items[0]?.classList.contains('acc-autocomplete-item-active')).toBe(true)
    items[2]?.dispatchEvent(new MouseEvent('mouseover', { bubbles: true }))
    await Promise.resolve()
    expect(items[2]?.classList.contains('acc-autocomplete-item-active')).toBe(true)
  },
}
