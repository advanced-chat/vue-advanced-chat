...
[private maintainer reporting route](https://github.com/advanced-chat/advanced-chat-components/security/advisories/new).
Prefix the report title with `Code of Conduct report:` so it is not triaged as a vulnerability. Do
not include sensitive conduct reports in public issues or Discussions. Security vulnerabilities
use the same private route but must follow [`SECURITY.md`](SECURITY.md).

Reports will be reviewed as maintainers are available; no response time is guaranteed. Community
leaders will respect the privacy and security of the reporter to the extent possible, but cannot
promise confidentiality where disclosure is required by law or necessary to address an immediate
safety risk.
...
at the [Contributor Covenant translations page](https://www.contributor-covenant.org/translations).
